...
$(function () {
    ...
    $("#code").live('click', codes)
    form.on('submit(*)', function (data) {
        $.ajax({
            url: '../../../asmx/Login.asmx/CheckLogin',
            type: 'post',
            ...
            success: function (data) {
                if ($(data.d).length != 0) {
                    $.each(data.d, function (index, e) {
                        $.cookie('uid', e.UserID, { path: '/' });
                        $.cookie('uname',escape(e.UserName), { path: '/' });
                        $.cookie('rid', e.RoleID, { path: '/' });
                    })
                    window.location.href = "../../View/Main.html";
                } else {
                    layer.msg('用户名或密码错误', { icon: 6 });
                }
            }
        })
    })
